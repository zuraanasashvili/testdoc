# პროდუქტების ცხრილის აღწერა

![ცხრილი](images/products.png)

## შესავალი

პროდუქტების გვერდზე მომხმარებელი ხედავს პროდუქტების კატალოგთან დაკავშირებულ ყველა ინფორმაციას.
პროდუქტების გვერდზე მომხმარებელს შეუძლია ისეთი ქმედებების გაკეთება, რომელიც დაკავშირებულია პროდუქტებთან, პროდუქტების ფასებთან ან მარაგებთან.

## ცხრილის ფუნქციონალი

### სვეტები

![სვეტები](images/products_general.png)

პროდუქტების გვერდზე არის ცხრილი, რომლის სვეტებია:

- ჩანაწერების მონიშვნა
- რჩეულებში დამატება
- სურათი
- დასახელება/ბარკოდი
- მომწოდებელი
- კატეგორია
- რაოდენობა
- შეს. ფასი
    - შესყიდვის ფასის კალკულაცია ხდება საშუალო შეწონილის ფორმულით - sumproduct(რაოდენობა, ღირებულება)/sum(რაოდენობა)
- გასაყიდი ფასი
- ფასნამატი
- დღგ

ცხრილის **ყველა** სვეტზე (მომწოდებლის გარდა, ვინაიდან შესაძლოა ერთ პროდუქტს ქონდეს ერთზე მეტი მომწოდებელი) შესაძლებელია სორტირება და ფილტრაცია.
მომხმარებელს შეუძლია სვეტების ასარჩევი ელემენტიდან **მხოლოდ** შემდეგი სვეტების ჩართვა / ამორთვა:

- შეს. ფასი
- გასაყიდი ფასი
- ფასნამატი
- დღგ

ცხრილში, თითოეულ ჩანაწერზე, მომხმარებელს შეუძლია:

- მონიშვნა
- რჩეულებში დამატება / რჩეულებიდან წაშლა
- პროდუქტის View-ს ნახვა
- მომწოდებლის View-ს ნახვა
- კატეგორიის View-ს ნახვა

### ჩანაწერების მონიშვნის ფუნქციონალი

![მონიშვნა](images/select.png)

ჩანაწერების მონიშვნას აქვს სამი მდგომარეობა

- თუ არცერთი ჩანაწერი არ აქვს მომხმარებელს მონიშნული შესაძლებელია დაკლიკვით ყველა ჩანაწერი მონიშნოს
- თუ ერთი ჩანაწერი აქვს მომხმარებელს მონიშნული, დაკლიკვით შეუძლია ყველა ჩანაწერი მონიშნოს
- თუ ყველა ჩანაწერი აქვს მომხმარებელს მონიშნული, დაკლიკვით შეუძლია ყველა ჩანაწერს მონიშვნა გაუთიშოს

### რჩეულების ფუნქციონალი
![რჩეულები](images/favorites.png)

რჩეულების ფუნქციონალს აქვს სამი მდგომარეობა

- თუ ცხრილში მომხმარებელს არცერთი პროდუქტი არ აქვს რჩეულში, რჩეულების ღილაკზე დაჭერით ყველა ჩანაწერი გახდება რჩეული
- თუ ცხრილში მომხმარებელს ერთი პროდუქტი მაინც აქვს არჩეული, რჩეულების ღილაკზე დაჭერით ყველა ჩანაწერი გახდება რჩეული
- თუ ცხრილში ყველა ჩანაწერი არის რჩეული, რჩეულების ღილაკზე დაჭერით მოხდება ყველა რჩეული ჩანაწერის დეაქტივაცია

რჩეულებში დამატება / დაეაქტივაცია დამოკიდებულია pagination-ზე. მაგ: თუ ცხრილში, არის მხოლდ 10 ჩანაწერი, რჩეულებში ყველას მონიშნის შემთხვევაში, მხოლოდ 10 ჩანაწერი დაემატება რჩეულებს

ჩანაწერის რჩეულიდან დეაქტივაციისას, გამოდის პოპაპ ფანჯარა:

![დეაქტივაცია](images/deactivation.png)

### View

ნებისმიერ ჩანაწერზე დაჭერისას მომხმარებელი ხედავს პროდუქტის view-ს. View-დან შესაძლებელია პროდუქტის რედაქტირება, წაშლა და ინფორმაციის ნახვა

![view](images/view.png)

პროდუქტების ცხრილში, მომწოდებლის ჩანაწერზე დაჭერისას, მომხმარებელი ხედავს მომწოდებლის view-ს. View-დან შესაძლებელია მომწოდებლის რედაქტირება, წაშლა და ინფორმაციის ნახვა

![view-supplier](images/view-supplier.png)

პროდუქტების ცხრილში, კატეგორიის ჩანაწერზე დაჭერისას, მომხმარებელი ხედავს კატეგორიის view-ს. View-დან შესაძლებელია კატეგორიის რედაქტირება, წაშლა და ინფორმაციის ნახვა

![view-category](images/view-category.png)


### ჰოვერის ფუნქციონალი

![ჰოვერი](images/hover.png)

ცხრილის ჩანაწერებზე კურსორის მიტანისას (ჰოვერზე) მომხმარებელს შეუძლია შემდეგი სამი ექშენის გაკეთება:

- დამალვა / გამოჩენა (თუ ჩანაწერი დამალულია - გამოჩენა, თუ გამოჩენილი - დამალვა)
- რედაქტირება
- წაშლა

დამალული პროდუქტები არ ჩანან პოს-ში

### ღილაკები

![ღილაკები](images/buttons.png)


#### დამატება

![დამატება](images/add_button.png)

- **პროდუქტების დამატების ღილაკი ჩანს, როდესაც არცერთი ჩანაწერი არ არის მონიშნული**

პროდუქტების დამატების შესახებ ინფორმაციის ნახვა შესაძლებელია [პროდუქტების დამატების](add_products.md) გვერდზე და [პროდუქტების ატვირთვის გვერდზე](#_14)

#### რედაქტირება

![რედაქტირება](images/edit_button.png)

- **პროდუქტების რედაქტირების ღილაკი ჩანს, როდესაც ერთი ჩანაწერი არის მონიშნული**

პროდუქტების რედაქტირების შესახებ ინფორმაციის ნახვა შესაძლებელია [პროდუქტების რედაქტირების](edit_products.md) გვერდზე

#### წაშლა

![წაშლა](images/delete_button.png)

- **პროდუქტების წაშლის ღილაკი ჩანს, როდესაც ერთი ან ერთზე მეტი ჩანაწერი არის მონიშნული**

პროდუქტების წაშლის შესახებ ინფორმაციის ნახვა შესაძლებელია [პროდუქტების წაშლის](delete_products.md) გვერდზე

#### ფასების სინქრონიზაცია

![ფასების სინქრონიზაცია](images/price_synch.png)

- **ფასების სინქრონიზაციის ღილაკი ჩანს, როდესაც არცერთი ჩანაწერი არ არის მონიშნული**

ფასების სინქრონიზაციისას მომხმარებელი ირჩევს ფილიალს, რომლიდანაც არსებულ ფილიალში მოხდება ფასების გადმოტანა. მაგ: თუ B ფილიალში პროდუქტ 12345-ზე ფასი არის 5₾, ხოლო A ფილიალში - 3₾, ფასების სიქრონიზების შემდეგ, A ფილიალში პროდუქტის ფასი გახდება 5₾

#### ფასების ცვლილება

![ფასების ცვლილება](images/price_change.png)

- **ფასების ცვლილების ღილაკი ჩანს, როდესაც არცერთი ჩანაწერი არ არის მონიშნული**

პროდუქტებზე ფასების ცვლილების შესახებ ინფორმაციის ნახვა შესაძლებელია [ფასების ცვლილების](price_change.md) გვერდზე

#### RS-დან ჩამოტვირთვა

![RS-დან ჩამოტვირთვა](images/rs_import_products.png)

- **RS-დან ჩამოტვირთვის ღილაკი ჩანს, როდესაც არცერთი ჩანაწერი არ არის მონიშნული**

RS-დან პროდუქტების ჩამოტვირთვისას, მომხმარებელს შეუძლია (**მხოლოდ იმ შემთხვევაში, თუ RS-ის პარამეტრები გაწერილია**) RS-დან მის ექაუნთთან დაკავშირებული პრიდუქტების ჩამოტვირთვა. მომხმარებელს შეუძლია ჩამოტვირთოს 3, 6, 9, 12 თვის პროდუქტები. 

#### პროდუქტების ატვირთვა

![პროდუქტების ატვირთვა](images/import_products.png)

- **პროდუქტების ჩამოტვირთვის ღილაკი ჩანს, როდესაც არცერთი ჩანაწერი არ არის მონიშნული**

პროდუქტების ატვირთვის ფუნქციონალს ორი ძირითადი გამოყენება აქვს. პროდუქტების ატვირთვით შესაძლებელია როგორც პროდუქტების ატვირთვა, ასევე არსებულ პროდუქტებზე ფასების ცვლილება.
ფუნქციონალის გამოყენებისას, აუცილებელია მომხმარებელმა პირველ რიგში ჩამოტვირთოს პროდუქტების ატვირთვის ფორმა, შეავსოს და ფორმის ატვირთვის გამოყენებით, შევსებული ფაილი ატვირთოს. აუცილებელია მომხმარებელმა პროდუქტების ასატვირთი ექსელის ფაილი, ფორმა, არ დააზიანოს (მაგ: ამოაკლოს სვეტები, წაშალოს ან დაუმატოს ფაილს ახალი გვერდები, ა.შ) იმ შემთხვევაში, თუ მომხმარებელი ასატვირთ ფაილს დააზიანებს, არ მოხდება ფაილის ატვირთვა და გამოვა შეტყობინება: **დაფიქსირდა შეცდომა**.
<!-- თუ მომხმარებელი ატვირთავს პროდუქტს, რომლის ბარკოდიც ოპტიმოში უკვე არსებობს და ექსელში განსხვავებული ფასია, მოხდება ფასის ცვლილება. -->
სტრუქტურულად სწორი ფაილის ატვირთვის შემთხვევაში, შესაძლებელია დატიდან გამომდინარე პროდუქტები წარმატებით აიტვირთოს ოპტიმოში, შეიცვალოს ფასები ან გამოვიდეს პოპაპ ფანჯარა

![პროდუქტები ვერ აიტვირთა](images/import_products_popup.png)
ჩამოტვირთვა ღილაკზე დაჭერით მომხმარებელი ჩამოტვირთავს ექსელის ფაილს, რომელშიც შეუძლია მის მიერ შევსებული დატა გააანალიზოს ვალიდაციების ჭრილში

![result.xlsx](images/result.png)

ჩამოტვირთულ ექსელის ფაილში შესაძლებელია შევხვდეთ სტრიქონებს მწვანე, ყვითელ, ლურჯ და წითელ ფერებში:

- სწორად შევსებული პროდუქტები, რომლების ატვირთვაც წარმატებით მოხდა, მწვანედ არიან გაფერადებული
- დუბლირებული პროდუქტები, რომლებიც ვერ აიტვირთა, ყვითლად არიან გაფერადებული (თუ ექსელის ფაილში მაქვს 1-ზე მეტი ერთი და იმავე ბარკოდის პროდუქტი, დუბლირებაა. ასევე დუბლირებაა, თუ ექსელში მაქვს ბარკოდი X, ოპტიმოშიც მსგავსი ბარკოდის პროდუქტი უკვე არსებობს და ერთი და იგივე გასაყიდი ფასი აქვს ექსელსა და ოპტიმოში)
- ფასშეცვლილი პროდუქტები, ლურჯად არიან გაფერადებული (**იმისთვის რომ ფასის ცვლილება მოხდეს, მომხმარებელს შემოყავს ოპტიმოში არსებული ბარკოდის პროდუქტი ოპტიმოსგან განსხვავებული გასაყიდი ფასით**)
- პროდუქტები, რომლებიც ვერ აიტვირთა, წითლად არიან გაფერადებული

#### ექსპორტი სასწორისთვის 

![ექსპორტი სასწორისთვის](images/scales_export.png)

- **ექსპორტი სასწორისთვის ღილაკი ჩანს, როდესაც არცერთი ჩანაწერი არ არის მონიშვნული**

სასწორის ექსპორტის ფუნქციონალის გამოყენებისას, მომხმარებელს შეუძლია აირჩიოს

- DIGI SM-320 - ამ ოფშენის არჩევისას, ჩამოიტვირთება ექსელის ფაილი, რომლის შეტვირთვაც შესაძლებელია DIGI SM-320 სასწორზე. ჩამოტვირთულ ფაილში სტრიქონების რაოდენობა ყოველთვის არის 6500
- Shrih-Print M - ამ ოფშენის არჩევისას, ჩამოიტვირთება ტექსტური ფაილი, რომლის შეტვირთვაც შესაძლებელია Shtrih-Print M სასწორზე. ჩამოტვირთულ ფაილში სტრიქონების რაოდენობა ყოველთვის არის 9999

#### ექსელში ექსპორტი

![ექსელში ექსპორტი](images/excel_export.png)

- **ექსელში ექსპორტის ღილაკი ჩანს, როდესაც არცერთი ჩანაწერი არ არის მონიშნული**

ექსელში ექსპორტის ღილაკზე დაჭერისას, მომხმარებელი ჩამოტვირთავს ექსელის ფაილს, რომელშიც ფილიალის მონაცემების მიხედვით შესაძლებელია ყველა პროდუქტზე არსებული ინფორმაციის ნახვა:

- ბარკოდი
- საქონლის დასახელება
- ზომის ერთეული
- მომწოდებელი
- კატეგორია
- დღგ-ს ტიპი
- მიმდინარე მარაგი
- მინიმალური მარაგის ზღვარი
- თვითღირებულება
- გასაყიდი ფასი
- ფერი
- ზომა
- GroupId
- ფასნამატი


#### ბარკოდის ბეჭდვა

![ბარკოდის ბეჭდვა](images/print_barcode.png)

- **ბარკოდის ბეჭდვის ღილაკი ჩანს, როდესაც 1 ან მეტი ჩანაწერი არის მონიშნული**

ბარკოდის ბეჭდვის ფუნქციონალის ჭრილში გამოდის ბეჭდვის ფანჯარა, რომელშიც მომხმარებელი ხედავს მონიშნული პროდუქტის/პროდუქტების ფასმაჩვენებელს. მომხმარებელს შეუძლია ფანჯარაში ბეჭდვის ოფშენების საჭიროებისამებრ ცვლილება და დაბეჭდვა

<!-- #### არცერთი ჩანაწერის მონიშვნის შემთხვევაში ჩანს ღილაკები:

- [დამატება](add_products.md) 
- [ფასების სინქრონიზაცია](price_synch.md)
- [ფასების ცვლილება](price_change.md)
- [RS-დან ჩამოტვირთვა](rs_import_products.md)
- [პროდუქტების ატვირთვა](product_import.md) 
    - ფორმის ჩამოტვირთვა
    - ფორმის ატვირთვა
- [ექსპორტი სასწორისთვის](scales_export.md)
    - DIGI SM-320
    - Shtrih-Print M
- [ექსელში ექსპორტი](excel_export.md)

#### ერთი ჩანაწერის მონიშვნის შემთხვევაში ჩანს ღილაკები:

- [რედაქტირება](edit_products.md) 
- [წაშლა](delete_products.md) 
- [ბარკოდის ბეჭდვა](print_barcode.md) 

#### ერთზე მეტი ჩანაწერის მონიშვნის შემთხვევაში ჩანს ღილაკები:

- [წაშლა](delete_products.md)
- [ბარკოდის ბეჭდვა](print_barcode.md)  -->